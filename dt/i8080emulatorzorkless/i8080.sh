...
echo 'Quick and dirty i8080 emulator by NASZVADI Peter, 2019,2023,2025.'
...
if [[ -f "$ROMFILENAME" ]]
    ...
    echo 'Initializing rom with builtin 2k ROM basic'
...
219) if ((M[PC++]==0))
     then
         if IFS='' read -r -t 0 _ >/dev/null 2>/dev/null
         then
             ((A=255))
         else
             ((A=0))
     ...
     then
         if IFS='' read -d $'\0' -r -s -t 0.1 -n 1 REPLY >/dev/null 2>/dev/null
         then
             A=$(printf %d "'$REPLY") ; ((A=(A==10)?13:(A<0?(256-(-A)%256)%256:A%256)))
         else
             A=0
         fi
     else
